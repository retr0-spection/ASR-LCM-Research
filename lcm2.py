...
class LCM_MCB(nn.Module):
    ...
    def forward(self, x, attention_mask=None):
        ...
        for i in range(self.n_codebooks):
            ...
            Dh = D // H
            x_reshaped = x_emb.view(B, S, H, Dh).transpose(1, 2)
            x_flat = x_reshaped.transpose(1, 2).reshape(B, S, D)
...
def load_checkpoint_compatible(model, ckpt_path, device, ddp):
    ...
    # Handle DDP vs non-DDP key mismatches
    new_state_dict = {}
    ...
    # Load into model (respect whether it's wrapped or not)
...
def train_lcm(
    ...
    for epoch in range(start_epoch, epochs):
        ...
        total_loss = 0
        per_codebook_losses = (
            [0] * model.module.n_codebooks if ddp else [0] * model.n_codebooks
        )
        batch_times = []
        restart_epoch = False
            ...
            try:
                ...
                if "CUDA out of memory" in str(e):
                    ...
                    if per_step_batch_size > 2:
                        new_bs = per_step_batch_size // 2
                        logger.warning(
                            f"Reducing per-step batch size {per_step_batch_size} -> {new_bs} and restarting epoch."
                        )
                        per_step_batch_size = new_bs
                        restart_epoch = True
        ...
        if is_rank0:
            ...
            checkpoint_path = os.path.join(
                checkpoint_dir, f"lcm_epoch{epoch+1}.pt"
            )
            torch.save(
                {
                    "epoch": epoch + 1,
        ...
        wandb.log(
            {
                ...
                **{
                    f"codebook_{i}_loss": avg_per_codebook_losses[i]
                    for i in range(len(avg_per_codebook_losses))
                },
            }
        )
    ...
    logger.info(
        f"Training completed in {total_training_time/3600:.2f} hours. Final loss: {avg_loss:.6f}"
    )
